...
const BoardList = ({ orgId, query }: BoardListProps) => {
  ...
  if (data === null) throw new Error("No board found in database!");
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-5 mt-8 pb-10">
        {data?.map(board => (
          <BoardCard id={board._id} key={board._id} title={board.title} imageUrl={board.imageUrl} authorId={board.authorId} authorName={board.authorName} createdAt={board._creationTime} orgId={board.orgId} isFavourite={false} />
        ))}
      </div>
    </div>
  );
